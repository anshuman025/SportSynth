...
const App: React.FC = () => {
  ...
  const matchTypes = ['All', 'T20', 'ODI', 'Test', 'Other'];

  const filteredMatches = useMemo(() => {
    if (filterType === 'All') return matches;
    return matches.filter(m => (m.matchType || 'other').toLowerCase() === filterType.toLowerCase());
  }, [matches, filterType]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <main className="lg:col-span-2 space-y-6">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <h2 className="text-xl font-bold border-l-4 border-brand-blue pl-3 whitespace-nowrap">Current Matches</h2>
              <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide flex-nowrap">
                {matchTypes.map(type => (
                  <button
                    key={type}
                    onClick={() => setFilterType(type)}
                    className={`px-3 py-1 rounded-full text-xs font-bold border-2 border-black transition-all whitespace-nowrap
                      ${filterType === type ? 'bg-black text-white' : 'bg-white text-black hover:bg-gray-100'}`}
                  >
                    {type}
                  </button>
                ))}
                <span className="text-xs font-mono bg-black text-white px-2 py-1 rounded ml-auto flex-shrink-0">
                  Total: {isLoading ? '...' : filteredMatches.length}
                </span>
              </div>
            </div>
            {newMatchesCount > 0 && (
            ...
            {!isLoading && !error && filteredMatches.length === 0 && (
              <div className="p-12 text-center border-2 border-black rounded-2xl bg-gray-50">
                <p className="font-bold text-lg">No matches found for {filterType}</p>
              </div>
            )}
