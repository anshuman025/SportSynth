    ...
    `}>
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex flex-col gap-1">
          <span className="text-xs font-bold uppercase tracking-wider text-black bg-brand-yellow border-2 border-black rounded-full px-2 py-0.5 inline-block w-fit">
            {match.matchType && match.matchType !== 'other' ? match.matchType.toUpperCase() : match.sport.toUpperCase()}
          </span>
          {match.league && (
            <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wide line-clamp-1">
              {match.league}
            </span>
          )}
        </div>
      ...
      <div className="flex items-center justify-between mt-auto pt-4 border-t-2 border-gray-100 border-dashed">
        <span className="text-xs text-gray-500 font-medium whitespace-nowrap overflow-hidden text-ellipsis">
